...
namespace PTSLibrary.DAO
{
    public class AdminDAO : SuperDAO
    {
        public int Authenticate(string username, string password)
        {
            ...
            SqlDataReader dr; sql = String.Format("SELECT UserId FROM Person WHERE IsAdministrator = 1 AND Username='{0}' AND Password='{1}'", username, password);
        ...
        public List<Team> GetListOfTeams()
        {
            ...
            try
            {
                ...
                {
                    Team t = new Team((int)dr["TeamId"], dr["Location"].ToString(), dr["Name"].ToString(), null);
                    teams.Add(t);
                }
                dr.Close();
            }
